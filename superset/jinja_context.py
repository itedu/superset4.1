...
class ExtraCache:
    ...
    ##自定义的函数
    def current_user_address(self) -> Optional[str]:
        from superset.extensions import db
        from superset.custom.models import Custom
        custom = db.session.query(Custom).filter(Custom.user_id == g.user.id).first()
        ...
        self._context.update(
            {
                ...
                "current_user_address": partial(safe_proxy, extra_cache.current_user_address),
            }
        )
